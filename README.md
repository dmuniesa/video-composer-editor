...
2. **Analyzes every clip with AI**: description, 1–10 score, hashtags — plus optional **mood** (emotional tone), **energy** (motion level) and **scene context** (scene / time of day / shot type), each toggleable in Settings; if a clip already has named people, the AI uses their names in the description. With `agy` the analysis can watch the clip itself (a low-res MP4, clips up to 3 min) instead of sampled frames — it then also suggests the clip's **best moments as time ranges**, savable as trim ranges in one click. Via Google's [Antigravity CLI](https://antigravity.google/product/antigravity-cli) (`agy`, Gemini) or any **OpenAI-compatible endpoint** (z.ai GLM, OpenAI, OpenRouter, Ollama…), selectable on the in-app Settings page.
...
| **Montage** | Drag clips (or their ranges) from the bin onto the tracks. Drag to move, edge-drag to trim, snapping to beats/sections (**S** toggles). **Space** previews audio + a jump-cut video preview. **Del** removes the selected clip. **Normalize** levels every clip's audio to a common loudness (EBU R128 ≈ −16 LUFS); right-click a clip's audio waveform to set its gain in dB. The track volume still works on top. |
